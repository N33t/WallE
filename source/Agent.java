...
public class Agent {
	...
	private InitialExploreNode initialRouting(Position startPosition, Position endPosition, JobManager.Job parentJob, boolean box) throws Exception {
		...
		int obstacleMap[][];
		obstacleMap = new int[GameMap.MAX_COLUMN][GameMap.MAX_ROW];
		for (int i = 0; i < GameMap.MAX_COLUMN; i++) {
			for (int j = 0; j < GameMap.MAX_ROW; j++) {
	...
	public Plan createPlan(JobManager.Job job) throws Exception {
		...
		if (job != null) {
			...
			if (job.jobType == 'g') {
				...
				System.err.println("Path to avoid = " + job.path);
				//Do initial
